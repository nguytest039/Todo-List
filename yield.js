...
const handleTableClick = () => {
    document.querySelector("#table-1 tbody").addEventListener("click", (e) => {
...
const utils = {
    ...
    getWeekDateRange(weekNum) {
        ...
        // Create a new Date object for calculation to avoid mutating firstDayOfYear
...
function processHourData(data, from, to, hours) {
    ...
    data.forEach((m) => {
        if (!machineData.has(m.machineNo)) {
            machineData.set(m.machineNo, {
                machineNo: m.machineNo,
                vals: Array(hours.length).fill(0),
        ...
        (m.data || []).forEach((day) => {
            ...
            (day.data || []).forEach((h) => {
                ...
                machine.vals[i] += Number(h.outputTotal ?? 0);
                machine.seen[i] = true;
...
function renderHourTableVM(vm) {
    ...
    addRow(tbody, "Gap", mtGaps, mtGapsSeen, {cellClassResolver: getHourCellClass});
    ...
    machines.forEach((r) => {
        addRow(tbody, r.machineNo, r.vals, r.seen, {cellClassResolver: getHourCellClass});
    });
}
